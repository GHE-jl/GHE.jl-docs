...
- `n` is the number of log-spaced nodes the optimization solves for `ĝ`, returned at
  every index `t` and interpolated from those nodes `gOpt(τ)` (see `set_nodes` to recover their
  positions, and Modeling theory for why a reduced node set is used).
- `c=2` enforces both a positive first derivative and a negative second derivative past roughly
  3 h into the record (see Modeling theory for the full constraint set, `c=1` or `c=0`
  relax it).

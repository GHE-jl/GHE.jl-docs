...
`convolution`). This is ill-posed as a direct inverse (small measurement noise in `Texp`
is amplified into large, unphysical oscillations in a naively recovered `g`), so
`deconvolution` instead poses it as a **constrained, regularized optimization problem**,
following Dion et al. (2022, 2023, 2024).

The optimization below relies throughout on the root-mean-square, `rms`, to turn a vector
...
the optimization solves for `g` at only `n` log-spaced node indices `τ` (`set_nodes` on
...
The `c` keyword of `deconvolution` selects which physical constraints are imposed on `g`

...
here is documented in the API reference.
...
`ground_load_profile` (which takes time in **hours**):
...
`fluid_temperature` ties it together. Pass the per-length load `Q ./ H`, the model, the
...
`Tin` and `Tout` straddle `Tf` by ``\pm Q/(2 V C_f)``. See Fluid temperature.

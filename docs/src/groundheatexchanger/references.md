...
- **Marcotte, D., & Pasquier, P.** (2008). Fast fluid and ground temperature computation for
  ...
  The FFT-based convolution behind `convolution`.
...
- **Beaudry, G., Pasquier, P., & Nguyen, A.** (2024). New formulations and experimental validation of
  ...
  The non-stationary convolution behind `convolution_ns`.
...
- **Claesson, J., & Javed, S.** (2011). An analytical method to calculate borehole fluid temperatures
  ...
  The mean-fluid-temperature framework underlying `fluid_temperature`.
...
- **Pasquier, P., Zarrella, A., & Labib, R.** (2018). Application of artificial neural networks to
  ...
  The `PublishedANN` model behind `short_term_response` and
  `short_term_nodes`.
...
- **Pasquier, P., & Marcotte, D.** (2020). Robust identification of volumetric heat capacity and
  ...
  The `DeepANN` model (wider validity ranges, the default) behind
  `short_term_response` and `short_term_nodes`.
...
- **Bernier, M. A., Pinel, P., Labib, R., & Paillot, R.** (2004). A Multiple Load Aggregation
  ...
  The synthetic annual load profile generated by `ground_load_profile`.

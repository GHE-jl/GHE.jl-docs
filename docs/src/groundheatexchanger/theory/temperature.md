...
`interp=false` to disable that and evaluate exactly. See g-function compression.
...
short-term transfer function is required. `outlet_transfer_function` builds the complete
...
ANN — `short_term_response` / `short_term_nodes`, either the wider-range
`DeepANN` (Pasquier & Marcotte, 2020; the **default**, 21-day horizon) or the original
`PublishedANN` (Pasquier, Zarrella & Labib, 2018; 7-day horizon) — to any long-term

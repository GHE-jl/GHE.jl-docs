...
`interp` keyword flows through the `fluid_temperature` overloads that take a ground model,
...
`pchip_interpolation` is exposed directly as a reusable `GroundHeatExchanger.jl` helper —

...
`ThermalResponseTest.jl` is registered in the Julia General registry, along with its dependency
`GroundHeatExchanger.jl`, so they all resolve automatically:
...
- **Tutorial** — loading a TRT log, running every interpretation method, step by step.
- **Interpretation theory**:
  - Overview — the superposition model both method families are built on, and how to
    choose between them.
  - First-order approximation — the four FOA regressions and their validity windows.
  - Model inversion — the generic bounded least-squares core and its five model wrappers.
...
>the heater turns on, strictly positive, uniformly spaced). `decompose_trt` produces this

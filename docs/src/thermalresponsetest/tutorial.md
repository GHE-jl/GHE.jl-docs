...
`load_trt_data` reads it, computes the mean fluid temperature, and (by default)
...
`decompose_trt` then splits it into heating and recovery phases:
...
>`TRTDataset` assumes a single heating phase followed by a single recovery phase. A test
...
`load_trt_data` uses the arithmetic mean by default. `mean_fluid_temperature`
...
`fit_ils_foa_T` regresses the heating-phase temperature against `log(t)` past the ILS
...
`fit_ils_foa_T_recovery` is the recovery-phase counterpart: it regresses against
...
- `centered_finite_difference` — a plain three-point finite difference.
- `bourdet_derivative` — Bourdet et al.'s (1989) log-spaced, noise-robust estimator; the
...
See First-order approximation for the equations and validity windows behind all four
methods.

## 6. Model inversion

#TODO: rewrite so that fit_ground_response is in first, and the wrapper afterwards. Similarly to the README.md
...
closed-form regime. `fit_ils` is the simplest case (the infinite line source, same model
...
Four more wrappers cover the other `GroundResponse.jl` models `fit_ics` (infinite
cylindrical source), `fit_fls` (finite line source, needs `H` and buried depth `D`), and the
moving (groundwater-advection) variants `fit_mils` / `fit_mfls`, which additionally
...
All five are thin wrappers around `fit_ground_response`, the generic core that drives any
...
See Model inversion for the superposition equation, the optimizer/AD defaults, and why a
`maxtime`-triggered `Failure` return code does not by itself indicate a bad fit.

## 7. Variable-power signals

Some TRT rigs don't hold power perfectly constant. `step_signal` segments a noisy

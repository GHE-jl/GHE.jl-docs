...
`load_trt_data` reads a four-column CSV, time, power, inlet and outlet temperature, into
...
`mean_fluid_temperature` directly afterwards for a different averaging convention.

## Splitting heating and recovery

`decompose_trt` splits the loaded `DataFrame` into a `TRTDataset` on a heating-power
...
in First-order approximation actually consumes. `data` is the
...
The derivative-based FOA methods (`fit_ils_foa_dT`, `fit_ils_foa_dT_recovery`) need
...
- `centered_finite_difference` — the plain three-point finite difference.
- `bourdet_derivative` — brackets each point at a fixed logarithmic spacing `δ` instead of
...
- `critical_time` — the Fourier-number threshold `tc = 5 rb²/(k/Cs)` for the
  temperature-based methods.
- `residence_time` — the time for fluid to travel down and back up the borehole, the unit
  the derivative methods express their window in (`[64 tr, 512 tr]` by default).

## Variable-power signals

`step_signal` segments a noisy, variable-power signal into a chosen number of constant

...
with ``\gamma`` the Euler–Mascheroni constant. `fit_ils_foa_T` regresses ``T`` against
...
`fit_ils_foa_T_recovery` mirrors the heating-phase iteration, offsetting the critical-time
check by ``\bar t``. Its intercept can estimates ``T_0`` directly, so unlike `fit_ils_foa_T`, it takes no
...
`fit_ils_foa_dT` works in log space, ``\ln t + \ln\dot T = \ln\!\big(q/(4\pi k)\big)``,
...
`fit_ils_foa_dT_recovery` again solves for `k` in one shot from the log-space average, with
...
`centered_finite_difference` or `bourdet_derivative` — see
Data & utilities for the two estimators. The `TRTDataset`-based overloads default to the

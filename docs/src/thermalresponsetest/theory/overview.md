...
See First-order approximation and Model inversion for each family's equations.
...
heater turns on), and strictly positive and uniformly spaced values (for the model-inversion methods, whose forward model is an FFT convolution). `decompose_trt` enforces this automatically via `:t_rel`.
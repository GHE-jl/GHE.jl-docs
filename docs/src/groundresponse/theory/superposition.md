...
wall). `borefield_radius` builds this matrix from the coordinate array `xy`, along with the
...
default used by `ground_response` for multi-borehole inputs. The initial linear solve falls
...
`bloc_matrix` methods (see Overview).

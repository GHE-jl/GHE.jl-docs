...
finite depth). It is implemented in `ils` and is also the large-radius limit of the ICS.
...
The implementation in `ics` integrates this kernel with adaptive Gauss–Kronrod quadrature
...
`fls` evaluates this with Gauss–Kronrod quadrature.
